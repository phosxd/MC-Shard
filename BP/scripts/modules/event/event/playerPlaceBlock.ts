...
function Callback(event:PlayerPlaceBlockAfterEvent) {
    ...
    (Object.values(Module.persisData.events) as Array<Event>).forEach(event => {
        if (event.eventId != 'playerPlaceBlock') {return};
        ...
        if (playerActor) {
            try {block.dimension.runCommand(`execute positioned ${StringifyVector(block.location)} run ${StringFormat(blockActor.command, env)}`)} catch {};
        };
    });
};
